...
export class StackSvgCreator {
	...
	add (content, relativePath) {
		...
		let iconId = basename(
			relativePath.split(sep).join(`_`).replaceAll(/\s/g, `-`),
		...
		let widthAttr = iconSvg.getAttribute(`width`)?.replaceAll(/[^0-9]/g, ``)
		let heightAttr = iconSvg.getAttribute(`height`)?.replaceAll(/[^0-9]/g, ``)
